...
class Promises
{
  ...
  public static function flatMap < S,T > (p:Promise<S>, f:S->Promise<T>):Promise<T>
  {
    ...
    function complete(r) {
      ...
      p1.onProgress(function (p) { res.progress(0.5 + p * 0.5); return null; });
    ...
    p.onComplete(complete)
     ...
     .onProgress(function (p) { res.progress(p * 0.5); return null; });
  ...
  {
    var res = new Promise();
    p.onComplete (function (x) { if (f(x)) res.complete(x) else res.cancel(); return null; })
     .onCancelled(res.cancel)
     .onProgress (res.progress);
    return res;
  }
  ...
  public static function liftPromiseF2 <A, B, C> (f:A->B->C):Promise<A>->Promise<B>->Promise<C> 
  {
    return function (a:Promise<A>, b:Promise<B>) {
      ...
      function progress (p:Float) { res.progress(p * 0.5); return null; }
      
      a.onComplete(function (r) { valA = Some(r); check(); return null; }).onCancelled(res.cancel).onProgress(progress);
      b.onComplete(function (r) { valB = Some(r); check(); return null; }).onCancelled(res.cancel).onProgress(progress);
  ...
  liftPromiseF3 <A, B, C, D> (f:A->B->C->D):Promise<A>->Promise<B>->Promise<C>->Promise<D>
  {
    return function (a:Promise<A>, b:Promise<B>, c:Promise<C>) {
      ...
      function progress (p:Float) { res.progress(p * (1 / 3)); return null; }
      
      a.onComplete(function (r) { valA = Some(r); check(); return null; }).onCancelled(res.cancel).onProgress(progress);
      b.onComplete(function (r) { valB = Some(r); check(); return null; }).onCancelled(res.cancel).onProgress(progress);
      c.onComplete(function (r) { valC = Some(r); check(); return null; }).onCancelled(res.cancel).onProgress(progress);
  ...
  liftPromiseF4 <A, B, C, D, E> (f:A->B->C->D->E):Promise<A>->Promise<B>->Promise<C>->Promise<D>->Promise<E>
  {
    return function (a:Promise<A>, b:Promise<B>, c:Promise<C>, d:Promise<D>) {
      ...
      function progress (p:Float) { res.progress(p * 0.25); return null; }
      
      a.onComplete(function (r) { valA = Some(r); check(); return null; }).onCancelled(res.cancel).onProgress(progress);
      b.onComplete(function (r) { valB = Some(r); check(); return null; }).onCancelled(res.cancel).onProgress(progress);
      c.onComplete(function (r) { valC = Some(r); check(); return null; }).onCancelled(res.cancel).onProgress(progress);
      d.onComplete(function (r) { valD = Some(r); check(); return null; }).onCancelled(res.cancel).onProgress(progress);
  ...
  :Promise<A>->Promise<B>->Promise<C>->Promise<D>->Promise<E>->Promise<F>
  {
    return function (a:Promise<A>, b:Promise<B>, c:Promise<C>, d:Promise<D>, e:Promise<E>) {
      ...
      function progress (p:Float) { res.progress(p * 0.2); return null; }
      
      a.onComplete(function (r) { valA = Some(r); check(); return null; } ).onCancelled(res.cancel).onProgress(progress);
      b.onComplete(function (r) { valB = Some(r); check(); return null; } ).onCancelled(res.cancel).onProgress(progress);
      c.onComplete(function (r) { valC = Some(r); check(); return null; } ).onCancelled(res.cancel).onProgress(progress);
      d.onComplete(function (r) { valD = Some(r); check(); return null; } ).onCancelled(res.cancel).onProgress(progress);
      e.onComplete(function (r) { valE = Some(r); check(); return null; } ).onCancelled(res.cancel).onProgress(progress);
